...
import { customerDetail } from "../../common/api/Notice";
...
function CustomerDetailPage() {
  ...
  const [notice, setNotice] = useState<CustomerDetail | null>(null);

  useEffect(() => {
    const fetchNoticeDetail = async () => {
      try {
        if (id) {
          const res = await customerDetail(id);
          setNotice(res);
        }
      } catch (error) {
      ...
      <div>
        ...
        <div className="flex">
          ...
          <div className="ml-[152px] w-full">
            <div className="text-[32px] mt-[49px]">고객의 소리</div>

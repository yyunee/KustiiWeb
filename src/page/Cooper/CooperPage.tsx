...
import { policy, policyOne } from "../../common/api/cooper";
...
function CooperPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await policy(currentPage);
      const {
        posts,
        total_pages,
  ...
  const fetchSearchData = async () => {
    try {
      const res = await policyOne(currentPage, search);
      const {
        posts,
        total_pages,
      }: { posts: IntroData[]; total_pages: number } = res;
    ...
    <div>
      ...
      <div className="flex">
        ...
        <div className="ml-[152px] w-full">
          <div className="text-[32px] mt-[49px]">정책·언론 동향 등</div>
